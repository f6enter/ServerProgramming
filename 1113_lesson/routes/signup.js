...
pool = mysql.createPool({
   ...
   * 
   * host : 데이터베이스 서버의 주소 (자신 컴퓨터라면 127.0.0.1 혹은 localhost)
...
// 회원가입 form으로 부터 전달받은 값을 처리하는 기능함수.
router.post('/', function(req, res, next) { // signup다음에 주소가 없기 때문에 '/' 로.
	/**
	 * form 태그를 활용하여 데이터를 전달 받을 때 기능 함수에서는
	 * req객체 안에 있는 body라는 데이터 이름의 오브젝트에서 데이터를 가져올 수 있다.
	 * 
	 * 참고)
	 * 주소 : req.params.데이터이름
	 * post : req.body.데이터이름
	 * 
	 * 예시)
	 * {usr : "raejin", pwd:"1111", repwd:"1111", gender:"남성"}
	 * 
	 * 위의 오브젝트에서 usr 이라는 데이터 이름의 값 'raegin'을 가져오려면
	 * 변수 = req.body.usr;
	 */
   ...
   */
    ...
    // con.query() 함수의 매개변수는 sql문을 작성한 문자열과,
       ...
       */
			...
			// 에러가 발생하지 않을 경우(정상적으로 sql문을 실행한 경우)
			// 처리를 완료하고(데이터베이스에 값을 저장)
			// 다른 주소로 이동하고 싶은 경우에는 res.redirect('웹페이지주소')함수를 쓴다.
			// res.redirect('http://www.naver.com');
